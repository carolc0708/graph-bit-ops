...
{
    for(int i=0; i<N; i++) { if(vec[i] != 2147483647 && vec[i] == 1) { printf("%d ", i); }else { printf("0 "); }}
    printf("\n");
}

__global__ void printResVecFloat(const float* vec, const int N)
{
    for(int i=0; i<N; i++) { printf("%f ", vec[i]); if(i%32 == 31) printf(" ");}
    printf("\n");
}
...
{
    for(int i=0; i<N; i++) {printf(vec[i]>0?"1":"0"); if (i%4==3) printf(" ");}
    printf("\n");
}
