...
//    free(h_cscColPtr);
    ...
    int gridDim_new = (int)ceil(cbrt((double)nblockrows/8));
    ...
    for (iter=1; iter<=MAX_ITER; iter++) {
        ...
        // mngf[u] = A x gf
        // mxv
        bmv4_sparse_full_cc<int, int><<<grid_new, 512>>>(tA, grandparent, min_neighbor_parent_temp, bsrRowPtr, bsrColInd, nblockrows);
...
//    free(h_cscColPtr);
   ...
   // get gridDim, this is to avoid nblockrows being larger than MAX_gridDim (65535?!)
    ...
    for (iter=1; iter<=MAX_ITER; iter++) {
        ...
        // mngf[u] = A x gf
        // mxv
        bmv32_sparse_full_cc<int, int><<<grid_new, 512>>>(tA, grandparent, min_neighbor_parent_temp, bsrRowPtr, bsrColInd, nblockrows);
//        printResVec<int><<<1,1>>>(min_neighbor_parent_temp, nrows);
        ewiseMin<<<(int)ceil(nrows/1024.0), 1024>>>(min_neighbor_parent, nrows, min_neighbor_parent, min_neighbor_parent_temp);
//        printResVec<int><<<1,1>>>(min_neighbor_parent, nrows);
