...
//    printf("orivec: \n"); printHostVec(B, (nblockrows * blocksize));
    ...
    float *fC;
    cudaMalloc((void**)&fC, (nblockrows * blocksize) * 1 * sizeof(float));
    setDeviceValArr<int, float><<<1,1>>>(fC, nblockrows * blocksize, 0);

    unsigned* tC;
    cudaMalloc((void**)&tC, nblockrows * sizeof(unsigned));
    setDeviceValArr<int, unsigned><<<1,1>>>(tC, nblockrows, 0);
...
//        bmv32_sparse_bin<int, float><<<grid, 32>>>(tA, tB, tC, bsrRowPtr, bsrColInd, nblockrows);
...
//        bmv32_sparse_new<int, float><<<grid_new, 1024>>>(tA, tB, fC, blocksize, nblocks, 1, bsrRowPtr, bsrColInd, nblockrows, nblocks);
    ...
    for (i=0; i<TEST_TIMES; i++) { // follow warp consolidation model (32 threads per block)

          bmv32_sparse_bin_masked<int, float><<<NB, NT>>>(tA, tB, tC, bsrRowPtr, bsrColInd, nblockrows, tB);
//        bmv32_sparse_bin<int, float><<<grid, 32>>>(tA, tB, tC, bsrRowPtr, bsrColInd, nblockrows);
//        reduce<<<(int)ceil(nblockrows/1024.0), 1024>>>(tC, nblockrows, succptr);
//        reduce_naive<<<1,1>>>(tC, nblockrows, succptr);
//        cudaMemcpy(&succ, succptr, sizeof(int), cudaMemcpyDeviceToHost);
//        if (prev_succ == succ) break;
//        prev_succ = succ;
        cudaMemcpy(tB, tC, nblockrows * sizeof(unsigned), cudaMemcpyDeviceToDevice);
    }

    bmvbin_timer.Stop();
    double bmvbin32_time = bmvbin_timer.ElapsedMillis();///double(TEST_TIMES);
...
//    printf("csrspmvvec: \n"); printResVec<float><<<1,1>>>(dY, nrows);
    ...
    printf("speedup-bin: %.3lf\n", cusparsecsrspmvfloat_time/bmvbin32_time);
