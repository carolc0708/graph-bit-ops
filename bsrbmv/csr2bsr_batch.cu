...
__global__ void printGlobalBSR(const int* bsrrowptr, const int* bsrcolind, const unsigned* bsrval,
...
__global__ void printGlobalBSRBlock(const unsigned* bsrval, const int blocksize, const int nblocks)
{
...
/**
* batch the process of csr2bsr
* assume csr val are only 0 or 1
*/
void csr2bsr_batch(const int* h_csrRowPtr, const int* h_csrColInd,
...
//        printf("[blockrow %d] temp_nnz: %d, temp_rowstart: %d, temp_rowend: %d\n", i, temp_nnz, temp_rowstart, temp_rowend);
        ...
        if (temp_nnz != 0)
        {
            ...
            ToBit32Col<float><<<dim3(CEIL(blocksize), CEIL(temp_nblocks * blocksize)), 32>>>(temp_bsrval,
...
//        printGlobalBSR<<<1,1>>>(bsrRowPtr, bsrColInd, bsrVal, blocksize, nblockrows, nblocks);
//        int k; std::cin >> k;
    ...
    // printout global bsr to verify
//    printGlobalBSR<<<1,1>>>(bsrRowPtr, bsrColInd, bsrVal, blocksize, nblockrows, nblocks);
//    printGlobalBSRBlock<<<1,1>>>(bsrVal, blocksize, nblocks);
}